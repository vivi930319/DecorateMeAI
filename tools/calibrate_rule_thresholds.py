...
做法：
    眉型 / 鼻型：單一特徵、三個類別 -> 直接 grid search 兩個切點。
    眼型：7 個類別、3 個特徵 -> 用淺決策樹（max_depth=3）學規則，再人工移植成 if-else。
...
def calibrate_eye():
    """眼型 7 類：用淺決策樹學規則，再把樹印成可以直接抄回程式碼的 if-else。"""
...
def main():
    ...
    calibrate_1d("nose_shape", "ratio_width",
                 low_label="窄鼻", high_label="寬鼻", mid_label="標準鼻",
                 grid=np.arange(0.26, 0.35, 0.002))

    calibrate_eye()

    print("\n\n對照（同一個 val set）：")
    print("  部位          現行規則式(壞掉)   CNN")
    print("  brow_shape         0.333       0.589")
    print("  eye_shape          0.143       0.378")
    print("  nose_shape         0.354       0.666")
