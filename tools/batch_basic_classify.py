"""用手寫規則式批次標註照片，輸出 CSV。原用途是為 CelebA bootstrap 初始標籤。

限制

它呼叫的是 `analyzer.get_face_shape()` / `get_eye_shape()` 這類手寫 if-else，
...
2026-07-31 踩過：拿本程式做部署驗證，看到「桃杏眼 → 鳳眼 9/11」而以為模型或 ROI
裁切壞掉，花了很久去查通道順序與裁切邏輯，最後發現 ROI 像素完全相同，
是工具本身未走模型那條路。規則式的眼型分支在 ear <= 0.327 一律回鳳眼，
那個「系統性偏移」正是它的正常行為。
