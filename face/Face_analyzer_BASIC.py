...
def pose_guidance(yaw: float, pitch: float, yaw_limit: float, pitch_limit: float) -> str:
    ...
    不要講左右。 第二版寫的是「你現在露出的是左臉，請把頭轉向你的左邊」，並且用
    「前半句讓使用者自行修正」來擔保左右慣例萬一反了也不會出事。那個擔保等於把
    難題丟回去：看的人要先判斷「左臉」是誰的左，再在**鏡像**的預覽畫面裡換算方向
    （自拍預覽有 transform: scaleX(-1)），然後才決定頭要往哪轉。三步推理，每一步
    都可能反過來，而做錯的代價是角度更偏、再重拍一次。

    改成對稱線索：講一個「調到對了就看得出來」的目標，方向由使用者自己收斂。
    兩邊臉頰一樣寬，在鏡像或非鏡像的畫面上都成立，所以左右慣例反不反都不影響。
    這也順便解開了跟 _detect_pose、PRO 即時掃描共用左右慣例的耦合——那三處先前
    必須一起改才不會互相矛盾。
    """
    hints = []
    if abs(yaw) > yaw_limit:
        hints.append("臉沒有正對鏡頭：請慢慢轉動頭，直到兩邊臉頰在畫面上看起來一樣寬")
    if abs(pitch) > pitch_limit:
        # pitch 的正負號在 InsightFace 不同版本並不一致，這裡不敢斷言是抬頭還是低頭，
        # 改講使用者一定做得到、而且講錯不了的動作。自拍時這個角度幾乎都是手機
        # 拿得比眼睛高或低造成的，所以直接講鏡頭高度，不講頭該仰還是低。
        hints.append("鏡頭沒有跟眼睛同高：請把手機移到眼睛的高度，下巴放平")
    if not hints:
