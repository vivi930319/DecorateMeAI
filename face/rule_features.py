...
所有特徵都是比值，不是像素長度：同一張臉拍遠拍近像素會變、比值不會。
"""
...
def nose_features(a) -> dict[str, float]:
    ...
    擴充原因：舊版鼻型只有 `ratio_width` 一個特徵——臉型有 10 個、眼型有 3 個，
    鼻型顯然是被漏掉的。而實測顯示把這一個數字接到 384 維 DINOv2 embedding 後面，
...
def _jawline_angles(a) -> dict[str, float]:
    ...
    跟舊版失敗的 _jaw_shape_features 差在測量的穩定度：那組要從 36 個外框點
    估曲率（三點外接圓），在那個取樣密度下雜訊比訊號大。這裡只量兩個明確
...
def _vertical_thirds(a) -> dict[str, float]:
    ...
    這是傳統面相學的「三庭」，也是這組特徵裡唯一的縱向資訊——
    舊版十個特徵全部在描述「哪裡寬」與「轉角尖不尖」，沒有一個描述上中下的分配。
