...
LABEL_ALIASES = {
    # 2026-07-30：杏仁眼與桃花眼合併為「桃杏眼」。理由不是資料不夠，是這兩類的界線
    # 不存在——內容相同卻標成不同類別的樣本裡，「杏仁眼 vs 桃花眼」有 7 組、
    # 「圓眼 vs 杏仁眼」17 組。合併後 macro 由 0.388 升到 0.523（見發展歷程規格書 §6.6）。
    #
    # 2026-07-31：細長眼併入鳳眼，眼型降為四類。§6.6 當初就量到「細長眼 vs 鳳眼」
    # 有 17 組內容相同卻標成不同類別，一直沒處理；tools/suggest_merges.py 在修好的
    # 資料上重跑，這一組互認率 20.4%、對稱 0.84（兩邊認錯一樣多 = 界線不存在的樣貌）。
    #
    # ⚠️ 瞇縫眼原本指向細長眼，這裡一起改成鳳眼——對照表是**單次查表、不會串接**，
    # 留著會產出一個不在 CANONICAL_LABELS 裡的值，然後在下面那道檢查炸掉。
    "eye_shape": {
        "丹鳳眼": "鳳眼", "瞇縫眼": "鳳眼", "細長眼": "鳳眼",
        "杏仁眼": "桃杏眼", "桃花眼": "桃杏眼",
    },
    "nose_shape": {"窄鼻": "標準鼻", "蒜頭鼻": "寬鼻"},
    # 2026-07-30：M型唇併入花瓣唇。混淆矩陣顯示 M型唇 recall 只有 0.30——108 張裡
    # 只有 32 張判對，其餘均勻散到另外四類，那是「這個類別沒有可辨識特徵」的樣貌。
    # 兩者的共同點是唇峰明顯，差異在照片上分不出來。合併後這一類 recall 約 0.57。
    "lip_shape": {"M型唇": "花瓣唇"},
}
