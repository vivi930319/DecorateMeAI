...
lip_logic = {
    ...
    "花瓣唇": "上M下W，飽滿立體。建議在下唇W處疊加透明唇釉，打造如花瓣盛開的層次感。",
    "微笑唇": "嘴角自然上揚。適合勾勒唇角上揚線條，加強那種不笑也迷人的親和力。",
    "厚唇（嘟嘟唇）": "上下唇飽滿豐厚。適合模糊唇周邊界，打造霧氣氤氳或彈潤飽滿的誘惑感。",
    "薄唇": "線條利落氣質清冷。建議稍微外擴唇線 1-2mm，再用亮面質地增加視覺澎潤度。"
}
...
# 3. 核心處理函式
def get_customized_makeup_advice(style_name, face_shape, lip_shape, skin_detail):
    ...
    system_instruction = (
        ...
        "【情境導引任務】：\n"
        "1. 不要死板地讀出技巧。例如：看到『橫向刷腮紅』，請說『親愛的，我們讓腮紅在臉頰橫向輕舞，縮短臉部比例』。\n"
        "2. 針對唇形特色，請描述出唇部立體或飽滿的動人質感。\n"
        "3. **絕對禁止出現任何英文**。術語必須正確（唇釉、修容、打亮、腮紅）。\n"
        "4. 多用動詞：勾勒、暈染、輕觸、點綴。語氣多用助詞（喔、呢、囉、呀）。"
    )

  
    prompt_content = f"""
    請根據以下素材，為女孩寫一段充滿畫面感的客製化建議：
    
    【核心風格】：{style_name} ({style['description']})
    【臉型修正建議】：{face_shape} — {face_logic.get(face_shape, "")}
    【唇形魅力重點】：{lip_shape} — {lip_logic.get(lip_shape, "")}
    【膚色色彩關鍵】：{skin_logic.get(skin_detail, "")}
    【基礎底色參考】：眼妝{style['eye']}，底妝{style['base']}，唇彩建議{style['lip']}。
