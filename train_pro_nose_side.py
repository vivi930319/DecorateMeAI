...
# 朝天鼻資料夾的 65 個檔案、61 個唯一內容，**全部**也存在於翹鼻資料夾，
...
def collect_images() -> list[tuple[Path, str]]:
    ...
    先前這裡只有一條規則——「先看到誰算誰」。而迴圈是 `sorted(ROOT.iterdir())`，
    所以「先看到誰」完全由**資料夾名的字母序**決定。
    ...
    **翹鼻資料夾有 200 張，實際只有 133 張進訓練，而歸屬是字母序決定的，
    不是任何標註決定。** docstring 原本寫「0 組跨類衝突」，那個描述早就過期了。
