"""眼型分類（BASIC）：MobileNetV3-Small 吃眼周 ROI，5-fold 按人分組。

這個模型現在的狀況（2026-08-13 的評估報告）
-------------------------------------------
    accuracy 0.580｜macro-F1 0.569｜κ 0.419

    下垂眼  P 0.545  R 0.555  n=110
    圓眼    P 0.540  R 0.508  n=120
    桃杏眼  P 0.550  R 0.570  n=221
    鳳眼    P 0.649  R 0.640  n=225

**這是五個部位裡最不穩的一個**：逐 fold accuracy 0.448～0.674，標準差 ±0.077，
代表換一批人結果就跳，泛化能力有疑慮。主要混淆是圓眼→桃杏眼 37%，
以及桃杏眼↔鳳眼互相各約 19%。

眼型難的地方是**判斷區域小、又容易被遮擋**（瀏海、眼鏡、化妝、半閉眼）。

可以動手試的方向
----------------
1. **提高眼區解析度**：目前 ROI 是 96×96。眼睛的細節（雙眼皮、眼尾角度）在這個
   尺寸下可能已經被壓掉。放大到 128 或 160 再比一次。
2. **水平翻轉是免費的資料**：左眼翻過來就像右眼，對眼型完全安全，等於樣本加倍。
3. **模擬遮擋的增強**：訓練時隨機遮住一小塊，逼模型在部分遮擋下也能判斷。
4. 專案裡另有幾何融合的嘗試（`basic_roi_shadow.py` 的眼型精密幾何融合），
   但註解指出眼型只用 3 個幾何特徵、明顯不足——這是可以接手做的題目。

用法
----
    .venv\\Scripts\\python.exe training\\basic_eye_shape.py
    .venv\\Scripts\\python.exe training\\basic_eye_shape.py --epochs 40 --focal --class-weight
"""
