"""眉型分類（BASIC）：MobileNetV3-Small 吃眉毛 ROI，5-fold 按人分組。

這個模型現在的狀況（2026-08-24，四類、去重後 422 張）
-----------------------------------------------------
數字出自 `train_basic_cnn_roi.py --parts brow_shape --architecture convnext_tiny --cv 5`
（這支 MobileNetV3 版本是輕量對照組，正式評估看那一支）：
...
先前版本記載的「落尾眉是垃圾桶類（R 0.592 > P 0.355）」已經不成立：那是三類時期
且訓練集含大量重複檔的狀況。2026-08-24 修掉重複之後（一字眉 220 個檔案其實只有
102 張獨立照片，見 prepare_roi_cache 的說明），落尾眉的 recall 反而是偏低的那一端。

**現在真正的瓶頸是彎月眉↔落尾眉這一對，而且它擋在資料上，不在模型上。**

下面三個方向 2026-08-24 全部試過了，結論寫在這裡免得再試一次
-----------------------------------------------------------
1. `--loss focal --class-weight balanced`：**沒用**。macro 降到 0.505，
   落尾眉只從 0.408 到 0.417，代價是一字眉從 0.647 崩到 0.500。
2. 換 convnext_tiny：已經是現在的正式架構，上面的數字就是它。
3. 回頭查標註：`find_label_errors.py` 用 CNN + DINOv2 取 out-of-fold 預測，
   門檻放寬到 0.60，**找到 0 張**。所以不是少數照片標錯——模型對整片邊界
   都沒把握，錯誤是均勻散開的，不是集中在幾個離群標籤上。

補做的第四項：`tools/brow_geometry_probe.py` 直接照定義量幾何。
落尾眉的定義是「眉尾低於眉頭」，但正規化後的 tail_rise 中位數是
落尾眉 -0.056、**彎月眉 -0.058**——彎月眉還更落尾，而且它 100% 是負的
（落尾眉 97%）。眉尾比眉頭低是人臉的普遍結構，不是這一類的特徵。
只用幾何跑決策樹 macro 只有 0.327，比 CNN 還差。

把兩類合併重訓可得 macro 0.637 ± 0.020（+0.112），但**依產品決策維持分開**，
理由與該次實驗的完整數據記在 prepare_roi_cache.LABEL_ALIASES 的註解。
要再改善這一對，該做的是重新定義判準並重標，不是繼續調模型。

用法
----
    .venv\\Scripts\\python.exe training\\basic_brow_shape.py
    .venv\\Scripts\\python.exe training\\basic_brow_shape.py --epochs 40 --focal
"""
