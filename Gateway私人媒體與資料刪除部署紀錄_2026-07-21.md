...
| 服務 | Revision／版本 |
|---|---|
| Firebase Hosting | `20260721-023903` |
| AI Gateway | `ai-gateway-00025-252`／`private-media-20260721-3` |
...
- 受保護 API 第一次回傳 401 時，前端會清除殘留 session、停止 Admin 自動刷新並回到登入頁；並行請求不會重複顯示登入過期視窗。
- 匿名 GCS 物件網址 403。
...
新版會在第一個 401 出現時自動停止背景資料載入、清除舊登入狀態並回到登入畫面。使用者只需用會員資料庫內有效的帳號密碼重新登入；登入成功後才會進入會員或 Admin 頁面。若 `/auth/login` 本身為 401，代表該次帳密被會員資料庫拒絕，不是 Gateway 斷線，也不能由前端繞過。
