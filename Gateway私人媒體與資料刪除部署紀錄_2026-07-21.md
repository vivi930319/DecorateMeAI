...
| 服務 | Revision／版本 |
|---|---|
| Firebase Hosting | `20260721-031014` |
| AI Gateway | `ai-gateway-00026-rnd`／`session-preflight-20260721-1` |
...
- Gateway 最新單元測試 14 項通過；前一輪 Gateway／Render 單元測試共 22 項通過。
...
- 正式頁面啟動前會先呼叫 `/auth/session`，同時驗證 Gateway 與會員資料庫兩層 HttpOnly session；匿名或失效狀態回 401，會員頁背景請求不會啟動。
...
部署後第一次測試請使用 `Ctrl+F5` 強制重新整理，確認載入的是 `20260721-031014`。舊狀態的預期行為是 Console 最多看到一次 `/auth/session 401` 後回到登入頁，不應再看到 points、tasks、check-in、saved-looks 連續 401。
