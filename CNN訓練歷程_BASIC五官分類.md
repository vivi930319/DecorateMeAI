...
1. **規則式在 brow / eye / nose 上實質失效**（見第 7 節），線上使用者現在拿到的這三個欄位
   幾乎是常數。**這是目前最嚴重的問題**，而且它影響的是正在使用產品的人，不是實驗數字。
2. **標註矛盾未清理**：33 個身分在同一部位有衝突標籤（見 4.1）。
3. **val set 太小**，分數波動 ±0.1，目前無法用來選模型或調參。
4. **CNN 仍未達門檻**（0.38~0.67 vs 0.70），雖然全面優於規則式。
5. **線上 `strict_angle=True` 會直接拒絕非正臉照片**（回 `None`），拒絕率未量化。
6. **線上 `MAX_IMAGE_SIZE=2048` 與訓練用的 1024 不一致**，模型轉正式輸出前要對齊。

---

## 10. 下一步建議（依 CP 值排序）

第 7 節的發現改寫了優先順序 —— 原本第一順位的「補規則式 baseline」已完成，
而它的結果讓「修規則式」變成比「調 CNN」更急的事。

1. **決定 brow / eye / nose 三個欄位怎麼辦**（最急，影響線上使用者）。三個選項：
   - **修規則式的閾值**：先查為什麼眉型永遠輸出彎月眉（多半是某個比值的門檻設得讓所有人都落在同一區間）。
     這是治本，但需要有人重新定義幾何判準。
   - **改用 CNN**：這三個部位 CNN 分別是 0.589 / 0.378 / 0.666，遠優於規則式的 0.333 / 0.143 / 0.354。
     雖然沒到門檻 0.70，但「不夠準」仍然遠好過「所有人都一樣」。
   - **誠實降級**：信心不足時不輸出該欄位，而不是給一個假答案。
