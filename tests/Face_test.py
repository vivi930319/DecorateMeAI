...
# 導入 FileResponse，讓 FastAPI 能夠直接託管並回傳本地的 index.html 網頁檔案
from fastapi.responses import FileResponse

# 初版上線 docker 先調整膚色文獻 之後慢慢微調
app = FastAPI()
...
# 新增根目錄路由：當瀏覽器輸入 http://localhost:8001/ 時，直接將 index.html 丟給前端
@app.get("/")
...
class FaceAnalyzer:
    def __init__(self, image_input):
        ...
        if not map(None, results.multi_face_landmarks) or not results.multi_face_landmarks:
    ...
    def get_nose_shape(self):
        ...
        nose_height = self._dist(168, 2)
        face_width = self._dist(234, 454)

        ratio_width = nose_width / face_width
        ratio_height = nose_height / self._dist(10, 152)
