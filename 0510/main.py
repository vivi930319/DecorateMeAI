...
# ⚠️ 安全性提醒：以下硬編碼金鑰僅建議在除錯階段暫時保留，
# 正式環境請移除，只依賴環境變數 SUGGESTION_API_KEY，
# 避免金鑰外流到原始碼倉庫造成風險。
ALLOWED_KEYS = {
    SUGGESTION_API_KEY,
...
def verify_api_key(x_api_key: Optional[str]) -> bool:
    if not x_api_key:
        logger.warning("[DEBUG 偵測] Gateway 送來請求，但完全未帶 X-API-Key Header！")
        return False

    logger.info(f"[DEBUG 偵測] 收到 Gateway 傳來的 X-API-Key 為: '{x_api_key}'")

    for valid_key in ALLOWED_KEYS:
        if hmac.compare_digest(valid_key.encode("utf-8"), x_api_key.encode("utf-8")):
            logger.info(f"[DEBUG 偵測] 金鑰比對成功！匹配到: '{valid_key}'")
            return True

    logger.warning(f"[DEBUG 偵測] 傳來的 Key '{x_api_key}' 不在允許清單中！")
    return False
...
# 🔍 新增：全域請求記錄中介層，在每個進站請求印出所有收到的 header 名稱，
# 方便判斷是 Gateway 根本沒送出 X-API-Key，還是中間某層代理把它濾掉了。
@app.middleware("http")
...
if __name__ == "__main__":
    if not SUGGESTION_API_KEY:
        logger.warning("提示：環境變數未設定 SUGGESTION_API_KEY，將啟用內建相容金鑰清單模式運行。")
