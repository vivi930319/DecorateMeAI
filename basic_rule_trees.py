...
臉型 07-30 已移出，改由 CNN 提供答案。眼型還在，但正式答案由 DINOv2 覆蓋
（見 `basic_roi_shadow.dinov2_first_parts`），樹只是那條路徑失效時的中間層。
⚠️ 而樹在眼型也輸 CNN 0.067，所以那個「中間層」其實是降級——尚未處理。
...
# 2026-07-30：**臉型移出**，改由 CNN 提供正式答案。
#
# 上面 docstring 引的那組數字（樹 0.544 vs CNN 0.477）是 07-24 在舊資料上量的。
# 資料統一、身分聚類重建、CNN 調到 40ep/6e-4 之後，同一套 5-fold 的結果反過來：
#
#     臉型  樹 0.462  vs  CNN 0.522     CNN +0.060
#     眼型  樹 0.428  vs  CNN 0.495     CNN +0.067   （眼型的正式答案是 DINOv2 0.542）
#
...
#
# 眼型留在這裡是因為它的正式答案由 DINOv2 覆蓋（見 basic_roi_shadow.dinov2_first_parts），
# 樹只是那條路徑失效時的中間層。⚠️ 但樹在眼型也輸給 CNN 0.067，
# 所以真要退場時應該退回 CNN 而不是樹 —— 這一項尚未處理。
RULE_TREE_PARTS = ("eye_shape",)
