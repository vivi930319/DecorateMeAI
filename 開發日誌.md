...
- **前端管理員身分收斂**：`AdminStore.isAdminProfile()` 已移除 email 白名單 / `admin@` fallback，現在只信任資料庫回傳的 `role` / `level`。
- **前端權限顯示收斂**：PRO / render / admin 相關顯示與頁面存取，改為只讀後端回傳的 `role` / `level` / `status` / `allowedPages`；前端 demo 付款 / 本機升級申請不再用來開通權限。
- **註冊流程 fail-closed**：`doRegisterAction()` 在 `register` 或 `sendOTP` 失敗時直接中止，不再錯誤顯示 OTP 畫面。
- **render 服務加 server-side rate limit**：`replicate_render_api.py` 現在除了 API key，還會做固定時間窗限流，避免繞過前端後直接打 Replicate。
...
- 今日 live / cloud 狀態：
  ...
  - `POST https://morning-deeper-kick-medium.trycloudflare.com/suggest`（帶 `X-API-Key`）可到達，最小測試直接回 `200 completed`，`fallbackUsed=false`
  - `GET https://quickly-tend-sun-rod.trycloudflare.com/health`：`200 OK`
  - `GET https://quickly-tend-sun-rod.trycloudflare.com/api/products`：`200 OK`
  - `POST https://quickly-tend-sun-rod.trycloudflare.com/recommend-products`（帶 LAB 測試資料）：`404`
  - `POST https://quickly-tend-sun-rod.trycloudflare.com/api/recommend-products`（帶 LAB 測試資料）：`404`
  - `POST https://quickly-tend-sun-rod.trycloudflare.com/api/login`：`400`
- 這次確認到的主要差異：
  - 前端 `config.local.js` 現在實際使用的 `memberDatabaseUrl` / `productUrl` 已改成 `quickly-tend-sun-rod`；昨日文件中仍引用舊的 `purpose-violin-conflict-header` / `div-oct-deposits-acer` 已過時
  - `memberDatabase` / `product` base URL 目前不是 DNS 失敗，而是可到達狀態；但商品推薦 endpoint 已和前端 `recommendPath` 漂移，阻塞點不只剩註冊 csrf
...
- 目前新的主要阻塞：
  ...
  - `memberDatabase` / `product` 目前 Cloudflare 回應 header 仍以 `http://127.0.0.1:5000` 為允許來源；若前端本機仍跑 `5500`，瀏覽器串接會被 CORS 擋住
  - Redis / Celery / RQ 或其他正式 queue 方案仍未落地
- 目前版本控制狀態：
  - `PythonProject12` 是 Git repo，今天沒有新的未追蹤專案檔
  - `web_frontend` 是獨立 Git repo，不是桌面 repo；目前未提交修改包含 `.gitignore`、`config.local.example.js`、`frontend_smoke_check.js`、`index.html`、`js/api.js`、`js/router.js`
