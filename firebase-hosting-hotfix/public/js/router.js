...
const PageInit = {
    ...
    compare() {
        ...
        const hasRenderedAfter = () => {
            const render = Router.analysisPackage?.render || {};
            return !!(render.afterImageUrl || render.afterImageDataUrl || render.makeupOutput?.imageUrl || render.makeupOutput?.imageDataUrl);
        };
        ...
        holdBtn.onpointerdown = async () => {
            if (!hasRenderedAfter()) {
                const ok = await runCompareRender({ source: 'hold' });
                if (!ok) return;
            }
            showAfter();
        };
        ...
        let isRenderingCompare = false;

        async function runCompareRender(opts = {}) {
            if (isRenderingCompare) return false;
            const pkg = Router.analysisPackage;
            ...
            if (holdBtn) {
                ...
                holdBtn.textContent = 'AI 渲染中...';
            }
            renderStatus.style.display = 'block';
            renderStatus.textContent = opts.source === 'hold'
                ? '首次對比需要先 AI 渲染妝容...'
                : '傳送照片給 AI 渲染服務...';
            ...
            try {
                ...
                AnalysisDraft.save(Router.analysisPackage);
                setCompareImage('after');
                renderStatus.textContent = '渲染完成，按住可查看妝後效果。';
        ...
        if (renderBtn) {
            renderBtn.onclick = async () => {
                await runCompareRender({ source: 'button' });
            };
        }
...
function showApp() {
    ...
    const page = ROUTE_PAGES.has(requestedPage) ? requestedPage : 'dashboard';
    if (!ROUTE_PAGES.has(requestedPage)) {
